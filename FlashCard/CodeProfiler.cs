...
namespace CodeArtEng.PerformanceTools
{
    /// <summary>
    /// Code performance monitor tool.
    /// </summary>
    internal sealed class CodeProfiler
    {
        ...
        static public void Stop(string name)
        {
            ...
            {
                Debug.WriteLine(name + " completed in " + (tTime - TimeKeeper[name]).TotalMilliseconds.ToString("F4") + " ms");
            }
            else
                Debug.WriteLine("CodeProfiler: Item [" + name + "] not exists.", "WARNING");
        }
        /// <summary>
        ...
        [Conditional("CodeProfiler")]
        static public void WriteDebugMessage(string message) { Debug.WriteLine(message); }
        /// <summary>
        ...
        {
            if (targetFile.EndsWith(".txt") == false) targetFile += ".txt";
